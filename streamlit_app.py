...
if 'legit_model' not in st.session_state:
    st.session_state.legit_model = True
...
if ModelType == 'Classification (Default)' and label_size > threshold:
    ...
    st.session_state.legit_model = False
...
if random_or_date == 'Random':
    ...
    try:
        ...
        st.session_state.legit_date = False
        st.error(
            f"""
...
if st.session_state.legit_model and col_to_drop.count(target_feature) < 1:
...
if col4.button('Train model!') and st.session_state.legit_model and st.session_state.legit_date and col_to_drop.count(target_feature) < 1:
    ...
    if ModelType == 'Classification (Default)' and train_over:
        ...
        st.balloons()
        st.subheader("MSE")
        st.markdown(str(mean_squared_error(y_test, pereds)))
